...
# wrangling for the urbanization plot
...
health_raw <- read_csv("data/vht-health.csv")
...
#build shiny app using above components
...
For more about urbanization and politics, check out [this feature from FiveThirtyEight](https://fivethirtyeight.com/features/how-urban-or-rural-is-your-state-and-what-does-that-mean-for-the-2020-election/) which uses the same data.

```{r urbanization, echo = FALSE, message = FALSE, warning = FALSE}
...
serverWC <- function(input, output){
  ...
  output$cloud <- renderPlot({
     tmp <- data_for_cloud() %>% 
      ...
      filter(!(word %in% c("coronavirus","new","trump","covid","19","u.s","review","york", "popular", "news", "recommended", "videos")))
...
uiSent <- fluidPage (
  ...
  tabPanel(
    title = "Sentiment Analysis Continued...",
    sidebarLayout(
      sidebarPanel(
        ...
        sliderInput("date2",
                    ...
                    min = ymd("2020-06-01"),
                    max = ymd("2020-11-03"),
                    value = c(ymd("2020-06-01"), ymd("2020-11-03")),
                    timeFormat = "%Y-%m-%d",
      ...
      mainPanel(plotOutput(outputId = "bing"),
                plotOutput(outputId = "nrc")))))


serverSent <- function(input, output) {
  ...
  bing_cluster <- reactive({
    data <- titles_expanded %>%
      ...
      group_by(Dates) %>%
      mutate(positive_emotions = sum(positive)) %>%
      mutate(negative_emotions = sum(negative)) %>%
      select(Dates, positive_emotions, negative_emotions) %>%
      distinct() %>%
      filter(Dates >= ymd(input$date2[1]), Dates <= ymd(input$date2[2]))
      
  })
  
  nrc_cluster <- reactive({
    data <- titles_expanded %>%
      filter(Dates >= ymd(input$date2[1]),
             Dates <= ymd(input$date2[2])) %>%
  ...
  output$bing <- renderPlot({
    tmp <- bing_cluster()
    fviz_cluster(kmeans(tmp[2:3], input$cluster, iter.max = 1000, nstart = 5),
                 data = tmp[2:3],
                 palette = "Set1", 
                 ellipse.type = "convex",
                 labelsize = 5,
                 main = "KMeans clustering with emotional directions",
                 ggtheme = theme_bw()
    )
  })
  ...
  output$nrc <- renderPlot({
    tmp <- nrc_cluster()
    nrc_svd <- svd(tmp[2:9],2,2)
    nrc_approx <- nrc_svd$u[,1:2] %*% diag(nrc_svd$d[1:2]) %*% t(nrc_svd$v[,1:2]) # matrix approximation
    nrc_kmeans <- kmeans(nrc_approx, input$cluster, iter.max = 1000, nstart = 5)
    fviz_cluster(nrc_kmeans, data = tmp[2:9],
                 palette = "Set1", 
                 ellipse.type = "convex",
                 labelsize = 5,
                 main = "KMeans clustering with emotional embeddings",
                 ggtheme = theme_bw()
    )
  })
  }

shinyApp(ui = uiSent, server = serverSent)
```
